...
**Perf branch.** For performance regressions: establish a baseline measurement (timing harness, profiler, query plan), then bisect. Measure first, fix second.
...
# Reference: HITL Loop Template

For last-resort human-in-the-loop debugging, use this bash script pattern:
...
The agent runs the script; the user follows prompts in their terminal. Captured values are printed as KEY=VALUE for the agent to parse.
